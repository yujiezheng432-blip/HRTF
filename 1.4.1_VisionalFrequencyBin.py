...
sofa_path = r"dataset/sofa/P0001_FreeFieldCompMinPhase_48kHz.sofa"
...
for name, (taz, tel) in targets.items():
    ...
    print(f"{name} 频率点数 (frequency bins):", len(freqs))

    H_L = np.fft.rfft(left)
    mag_L_db = 20 * np.log10(np.abs(H_L) + 1e-12)

    plt.figure()
    plt.plot(freqs, mag_L_db, linewidth=1.2, label="Left |H(f)| (dB)")
    plt.scatter(freqs, mag_L_db, s=10)  # ⭐ 标记每一个frequency bin

    if right is not None:
        H_R = np.fft.rfft(right)
        mag_R_db = 20 * np.log10(np.abs(H_R) + 1e-12)
        plt.plot(freqs, mag_R_db, linewidth=1.2, label="Right |H(f)| (dB)")
        plt.scatter(freqs, mag_R_db, s=10)  # ⭐ 标记每一个frequency bin
